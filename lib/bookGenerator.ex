defmodule Kata14.BookGenerator do
  ...
  def process_words([first, second, third | rest], trigram_map)
    ...
    third_words = Map.get(trigram_map, "#{first} #{second}", [])
    trigram_map = Map.put(trigram_map, "#{first} #{second}", [third | third_words])
  ...
  def do_make_sentence(_trigram_map, _word1, _word2, count) when count < 1 do
    ""
  end

  def do_make_sentence(trigram_map, word1, word2, count) do
